    ...
    public class PoseUseSample : MonoBehaviour
    {
        ...
        protected virtual void Start()
        {
            ...
            for (int i = 0; i < _poses.Length; i++)
            {
                ...
                if(i < _poses.Length / 2)
                {
                    _poseActiveVisuals[i].name = "LH";
                }
                else
                {
                    _poseActiveVisuals[i].name = "RH";
                }
...
/**
            GameObject t_self = GameObject.Find("TargetSelf");
            GameObject t_partner = GameObject.Find("TargetPartner");
            GameObject t_object_self = GameObject.Find("TargetObjectSelf");

            if (t_self != null)
            {
                _poseActiveVisuals[poseNumber].transform.position = visualsPos / hands.Length;
            }
            else if (t_partner != null) 
            {
                    //_poseActiveVisuals[poseNumber].transform.position = GameObject.Find("PuppetLH").transform.position;
                    _poseActiveVisuals[poseNumber].transform.position = GameObject.Find("PuppetRH").transform.position;   
            }
            
            else //Object Condition
            {
                    _poseActiveVisuals[poseNumber].transform.position = GameObject.Find("ExpressiveCube").transform.position;
            }
